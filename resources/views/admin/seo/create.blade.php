               ...
               <form action="{{ route('seo-pages.store') }}" method="POST" enctype="multipart/form-data">
                          ...
                          <label for="parent_category">Page Name</label>
                            <select class="form-control" name="page_id" id="page_id" style="height: 35px;">
                                ...
                                @foreach($pages as $page)
                                              <option value="{{$page->id }},0">{{ $page->title}}</option>
                                          
                                 @endforeach
                                 @foreach($subcategories as $sub)
                                              <option value="{{$sub->id }},2">{{ $sub->name}}</option>
                  ...
                  <div class="col-md-12">
                     <div class="form-group">
                              ...
                              <input type="text" class="form-control" id="meta_keyword" placeholder="Enter Meta Keyword" name="meta_keyword" value="{{ old('meta_keyword') }}">
